...
| :--- | :--- | :--- | :--- |
| **Maintenance** | `Fix` | **Suchfunktion-Flackern, Tastatur-Schließen und Bildschirm-Refresh bei Texteingabe behoben:** 1. Behebung des Schließens der virtuellen Tastatur und Fokuserlusts im Szenen-/Raster-Filter (`StashGrid`): In `FilterViewModel.setFilter` wurde `pager.value = null` bei Filterwechseln entfernt, wodurch `StashGridControls` nicht mehr unmounted wird und das Textfeld während der Eingabe dauerhaft fokussiert bleibt. Zudem wird `searchQuery` in `StashGridControls` an `dataType` statt `filterArgs` gebunden, um Resets während des Tippens zu verhindern. 2. Behebung von nicht gemerkten Debounce-Jobs (`Job? = null`) in `SearchPage`, `StashGrid` und `SearchForPage`, wodurch vorherige Debounce-Timer beim Eintippen weiterer Buchstaben nicht abgebrochen wurden und jeder Buchstabe parallele Abfragen auslöste. 3. `SearchViewModel` leert bei neuen Suchanfragen die Zeilen nicht mehr synchron vorab (`listOf()`), wodurch das plötzliche Verschwinden und Wiederauftauchen aller Ergebniszeilen während der Eingabe verhindert wird. Vorherige Hintergrundsuchen werden nun sauber storniert (`searchJob?.cancel()`). 4. Fokus-Management in `SearchPage` entkoppelt, sodass `SearchEditTextBox` einen eigenen `FocusRequester` verwendet und nicht mehr mit `LazyColumn.focusRestorer` kollidiert. | Medium |
